...
main()
{
	...
	vec2 pos_prev = ((vec2(ipos) + vec2(0.5)) * vec2(global_ubo.inv_width * 2, global_ubo.inv_height) + motion.xy) * vec2(global_ubo.prev_width / 2, global_ubo.prev_height);
	...
	vec4 temporal_moments_histlen_hf = vec4(0);
	{
		...
		{
			float inv_w_diff = 1.0 / temporal_sum_w;
