...
void ParticleRendererVK::submitParticles(ParticleEmitterHandlerVK* pEmitterHandler)
{
	...
	for (ParticleEmitter* pEmitter : pEmitterHandler->getParticleEmitters()) {
		...
		m_ppDescriptorSets[frameIndex]->writeUniformBufferDescriptor(pEmitterBuffer, 3);
		m_ppDescriptorSets[frameIndex]->writeStorageBufferDescriptor(pPositionBuffer, 4);
		ImageViewVK* pParticleTextureView = pParticleTexture->getImageView();
		m_ppDescriptorSets[frameIndex]->writeCombinedImageDescriptors(&pParticleTextureView, &m_pSampler, 1, 5);
